...
from css_data_synthesis_test.address_admin_divisions import (
    ADMIN_MUNICIPALITY_CITY_DISTRICT_MAP,
    ADMIN_MUNICIPALITY_DISTRICT_TOWN_MAP,
    ADMIN_REGION_CITY_DISTRICT_MAP,
    ADMIN_REGION_CITY_DISTRICT_TOWN_MAP,
)
...
COHERENT_REGION_CITY_DISTRICT_MAP = ADMIN_REGION_CITY_DISTRICT_MAP
COHERENT_MUNICIPALITY_CITY_DISTRICT_MAP = ADMIN_MUNICIPALITY_CITY_DISTRICT_MAP
COHERENT_REGION_CITY_DISTRICT_TOWN_MAP = ADMIN_REGION_CITY_DISTRICT_TOWN_MAP
COHERENT_MUNICIPALITY_DISTRICT_TOWN_MAP = ADMIN_MUNICIPALITY_DISTRICT_TOWN_MAP
COHERENT_REGION_OPTIONS = _build_region_options(COHERENT_REGION_CITY_DISTRICT_MAP)
COHERENT_MUNICIPALITY_OPTIONS = tuple(
    {"province": "", "city": city, "districts": districts}
    for city, districts in COHERENT_MUNICIPALITY_CITY_DISTRICT_MAP.items()
)
COHERENT_ADDRESS_ADMIN_OPTIONS = _build_admin_division_options(
    COHERENT_REGION_CITY_DISTRICT_TOWN_MAP,
    COHERENT_MUNICIPALITY_DISTRICT_TOWN_MAP,
)
...
ADDRESS_REQUIRED_LOCALITY_KEYWORDS = tuple(
    dict.fromkeys((*COMMUNITY_SUFFIXES, "社区", "家园", "新村", "村", "屯", "组", "队", "公馆", "绿洲", "国际", "园"))
)
...
ADDRESS_NATURAL_VILLAGE_OPTIONS = (
    "东庄",
    "西庄",
    "南庄",
    "北庄",
...
def generate_local_customer_address(scenario_id: str, address_style: str = "standard_residential") -> str:
    ...
    attached_no = rng.randint(1, 9)
    road = rng.choice(ADDRESS_ROAD_OPTIONS)
    lane = rng.choice(ADDRESS_LANE_OPTIONS)
    section = rng.choice(ADDRESS_SECTION_OPTIONS)
    natural_village = rng.choice(ADDRESS_NATURAL_VILLAGE_OPTIONS)
    ...
    if address_style == "rural_group_number":
        ...
        rural_templates = (
            f"{village}{group_no}组{house_no}号",
            f"{village}{natural_village}{house_no}号",
            f"{village}{natural_village}{group_no}组{house_no}号",
            f"{village}{house_no}号",
            f"{village}{group_no}队{house_no}号",
            f"{village}{natural_village}{house_no}号院",
        )
        return f"{prefix}{town}{rng.choice(rural_templates)}"
    if address_style == "landmark_poi":
        poi = rng.choice(ADDRESS_POI_OPTIONS)
        landmark_templates = (
            f"{community}{poi}旁{house_no}号",
            f"{road}{house_no}号{community}{poi}旁",
            f"{community}{poi}东侧{house_no}号",
            f"{community}{poi}后面{house_no}号",
            f"{community}{poi}附近{building_no}号楼{room_no}室",
        )
        return f"{prefix}{town}{rng.choice(landmark_templates)}"
    if address_style == "house_number_only":
        house_number_templates = (
            f"{community}{house_no}号",
            f"{road}{house_no}号{community}",
            f"{road}{house_no}号院{community}",
            f"{lane}{house_no}号{community}",
            f"{community}{house_no}号院",
            f"{community}{house_no}号附{attached_no}号",
            f"{community}东门{house_no}号",
        )
        return f"{prefix}{town}{rng.choice(house_number_templates)}"
    detail_templates = (
        ...
        f"{community}{section}{building_no}栋{unit_no}单元{room_no}室",
        f"{community}{section}{building_no}号楼{room_no}室",
        f"{community}{building_no}号楼{unit_no}门{room_no}室",
        f"{community}{building_no}号楼{floor_no}层{room_no}号房",
        f"{community}{building_no}栋{unit_no}单元{room_no}号房",
        f"{community}{building_no}号院{unit_no}单元{room_no}室",
        f"{community}{building_no}排{house_no}号",
        f"{community}{building_no}排{unit_no}号{room_no}室",
        f"{road}{house_no}号{community}{building_no}栋{unit_no}单元{room_no}室",
        f"{road}{house_no}号院{community}{building_no}号楼{room_no}室",
        f"{road}{house_no}号附{attached_no}号{community}{building_no}栋{room_no}室",
        f"{lane}{house_no}号{community}{building_no}栋{room_no}室",
        f"{community}{building_no}栋{unit_no}单元{room_no}",
    )
