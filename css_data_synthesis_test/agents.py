...
23. 如果用户在补充地址时夹带“我不知道街道/街道不清楚/不知道怎么说/你们查一下/不是说了吗”等非地址内容，只保留用户明确说出的地址片段；例如“珠海区啊街道，我不知道”只应提取“珠海区”，不能把“啊街道”“我不知道”写入 address_candidate 或 merged_address_candidate。
24. 如果用户只是表达自己不在、不是、已离开或不使用客服核对的旧地址/当前位置，且没有明确给出新的可登记地址片段，必须返回空字符串，granularity=non_address；绝不能把被否定的地点词、方位词或整句话当作新地址、社区、小区、城市或补充定位点。
25. 示例：
...
26. 如果用户给出了新的城市，且它与“客服正在核对或上下文中的地址”里的旧城市不同，那么旧城市、旧省份都必须视为作废，不能继承到 merged_address_candidate 里。绝不能输出“四川省扬州市宝应县”“广东省武汉市江夏区”这类真实中国行政归属错误的组合。
...
11. “科目”“科慕”“可么”“扣摸”是 COLMO 的常见同音/近音说法，提取品牌时统一输出 COLMO；用户说“克莱沃”时提取品牌为“克莱沃”。
...
8. “科目”“科慕”“可么”“扣摸”是 COLMO 的常见同音/近音说法，提取品牌时统一输出 COLMO；用户说“克莱沃”时提取品牌为“克莱沃”。
...
6. 如果用户原话只是在表白、寒暄、感谢、抱怨、问费用、问上门时间，或谈其他和当前产品归属问题无关的内容，必须返回空字符串；不要把这类答非所问归入“不清楚/其他”分支。例如 usage_scene 问使用场所时，用户只说“我爱你”“谢谢”“什么时候上门”，answer_key 必须为空。
   但只要用户语义上是在表达“自己不知道、记不清、不确定、无法提供、暂时说不上来、只能提供很泛的信息（如只知道是美的）”这类当前问题无法给出更具体答案的意图，这就是对当前问题的有效回答，不是答非所问，必须返回当前节点对应的 unknown 类 answer_key；不要要求用户原话一字不落命中某个固定短语。
7. 对 brand_or_series 节点要特别注意：
   ...
   - 用户表示 COLMO 或克莱沃，判为 brand_series.colmo
   - 如果用户表达的是疑似 COLMO 的中文音译、谐音、近音、口误说法，且该说法不是另一个明确市面品牌，也判为 brand_series.colmo；例如“科目/科慕”这类听起来接近 COLMO 的说法。
   - 如果用户表达的是疑似克莱沃的同音、近音、错别字、语音转写误差或口误说法，也判为 brand_series.colmo；例如“克来沃/科莱沃/克莱我/克来我”这类听起来接近克莱沃的说法。
