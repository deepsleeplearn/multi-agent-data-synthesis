...
class FrontendServerTests(unittest.TestCase):
    ...
    def test_punctuation_predict_endpoint_returns_original_text(self):
        self._login()

        response = self.client.post(
            "/api/punctuation/predict",
            json={"text": "我在江苏省苏州市，工业园区"},
        )

        self.assertEqual(response.status_code, 200)
        payload = response.json()
        self.assertTrue(payload["ok"])
        self.assertEqual(payload["input_text"], "我在江苏省苏州市，工业园区")
        self.assertEqual(payload["punctuated_text"], "我在江苏省苏州市，工业园区")
    ...
    def test_session_respond_keeps_first_round_user_text_unpunctuated(self):
        ...
        self.assertEqual(session["transcript"][0].text, user_text)
        self.assertEqual(session["trace"][0]["user_text"], user_text)
        self.assertEqual(payload["transcript"][0]["text"], user_text)
        self.assertEqual(payload["terminal_entries"][0]["text"], user_text)

    def test_session_respond_keeps_second_round_user_text_unpunctuated(self):
        self._login()
        start_payload = self.client.post(
            "/api/session/start",
            json={"scenario_id": "frontend_case"},
    ...
    def test_frontend_static_files_include_review_close_control(self):
        ...
        self.assertIn('id="auth-remotion-root"', index_response.text)
        self.assertIn('class="auth-brand"', index_response.text)
        self.assertIn("CustAnnoX", index_response.text)
        self.assertIn("客服标注平台", index_response.text)
        self.assertIn('/static/remotion-login.js', index_response.text)
        self.assertIn("选择该条数据所属链路", index_response.text)
        self.assertIn("测试配套信息", index_response.text)
        self.assertIn("查看并编辑该条数据携带的测试模式配套信息", index_response.text)
        self.assertIn("选择JSONL文件", index_response.text)
        self.assertIn('accept=".jsonl,application/x-ndjson"', index_response.text)
        ...
        self.assertIn("parseStrictRewriteJsonlRecords", app_response.text)
        self.assertIn("改写模式只支持上传 JSONL 文件", app_response.text)
        self.assertIn("第 ${lineNumber} 行必须包含非空 session_id", app_response.text)
        self.assertIn("第 ${lineNumber} 行 conversations 必须是数组", app_response.text)
        self.assertIn("只能包含 role、content 两个键", app_response.text)
        self.assertIn("rewriteIdKeyPreference = 'session_id'", app_response.text)
        self.assertIn("rewriteDialogueKeyPreference = 'conversations'", app_response.text)
        ...
        self.assertNotIn("if (validateRewriteAirLinkSelection(record, option)) return true", app_response.text)
        ...
        self.assertIn("appendRewriteContextEditField", app_response.text)
        self.assertIn("saveRewriteContextModal", app_response.text)
        self.assertIn("rewriteContextEditRecordIndex", app_response.text)
        self.assertIn("rewriteImportedRecords[recordIndex].test_mode_context", app_response.text)
        self.assertIn("测试配套信息已更新", app_response.text)
        self.assertNotIn("空气能链路节点", app_response.text)
        ...
        self.assertIn("authLoginReveal", style_response.text)
        self.assertIn("auth-remotion-root", style_response.text)
        self.assertIn("auth-brand", style_response.text)
        self.assertIn("authBrandReveal", style_response.text)
        self.assertIn("grid-template-columns: minmax(24rem, 1fr) minmax(22rem, 28rem)", style_response.text)
        self.assertIn("column-gap: clamp(1.5rem, 4vw, 5rem)", style_response.text)
        self.assertIn("margin-right: clamp(0.25rem, 1.4vw, 1.6rem)", style_response.text)
        self.assertIn("justify-self: end", style_response.text)

        remotion_source = Path("frontend/remotion/src/main.jsx").read_text(encoding="utf-8")
        self.assertIn("@remotion/player", remotion_source)
        self.assertIn("useCurrentFrame", remotion_source)
        self.assertIn("particleLayer", remotion_source)
        self.assertIn("const particles = Array.from({length: 58}", remotion_source)
        self.assertIn("particleBeam", remotion_source)
        self.assertIn("const activeDurationSeconds = 60 * 60 * 12", remotion_source)
        self.assertIn("const wave = (frame, speed, offset = 0)", remotion_source)
        self.assertIn("const useBrowserZoomScale = () =>", remotion_source)
        self.assertIn("window.devicePixelRatio", remotion_source)
        self.assertIn("styles.zoomLayer", remotion_source)
        self.assertIn("requestAnimationFrame(tick)", remotion_source)
        self.assertIn("const motionFrame = reducedMotion ? Math.floor(liveFrame / 3) : liveFrame", remotion_source)
        self.assertIn("const entryFrame = Math.min(liveFrame, 120)", remotion_source)
        self.assertIn("settledX + wave(motionFrame", remotion_source)
        self.assertNotIn("wordmarkWrap", remotion_source)
        self.assertNotIn("LetterTile", remotion_source)
        self.assertNotIn("Service dialogue review", remotion_source)
        self.assertNotIn("Sessions", remotion_source)
        self.assertNotIn("Link Paths", remotion_source)
        self.assertNotIn("Review QA", remotion_source)
        self.assertIn("loop", remotion_source)
        self.assertNotIn("loop={false}", remotion_source)
        self.assertIn("moveToBeginningWhenEnded={false}", remotion_source)
        self.assertIn("width: 'max(100vw, 177.78vh)'", remotion_source)

        remotion_response = self.client.get("/static/remotion-login.js")
        self.assertEqual(remotion_response.status_code, 200)
        self.assertIn("particleLayer", remotion_response.text)
